...
from roi_data_layer_mine.roidb import combined_roidb
from roi_data_layer_mine.roibatchLoader import roibatchLoader
...
def parse_args():
  ...
  parser.add_argument('--exp', default='baseimg_semi_AL')
  parser.add_argument('--num_query', default=10, type=int)
  parser.add_argument('--query_strategies', default='random', type=str)
...
if __name__ == '__main__':
  ...
  dataset_g = roibatchLoader(roidb_g, ratio_list_g, ratio_index_g, args.batch_size, \
                           imdb_g.num_classes, training=True)

  dataloader_g = torch.utils.data.DataLoader(dataset_g, batch_size=args.batch_size,
                                           sampler=sampler_batch_g, num_workers=args.num_workers)

  # TARGET WITHOUT FLIP
  cfg.TRAIN.USE_FLIPPED = False
  imdb_tnf, roidb_tnf, ratio_list_tnf, ratio_index_tnf = combined_roidb(args.imdb_target)
  train_size_tnf = len(roidb_tnf)

  print('%s: %d roidb entries' % (imdb_tnf.name, len(roidb_tnf)))

  sampler_batch_tnf = sampler(train_size_tnf, args.batch_size)

  dataset_tnf = roibatchLoader(roidb_tnf, ratio_list_tnf, ratio_index_tnf, args.batch_size, \
                           imdb_tnf.num_classes, training=True)

  dataloader_tnf = torch.utils.data.DataLoader(dataset_tnf, batch_size=args.batch_size,
                            sampler=sampler_batch_tnf, num_workers=args.num_workers)
  ...
  max_pool_size = int(len(dataloader_t) * 0.35)
  print('max pool size: %d' % max_pool_size)
  num_query = args.num_query
  if args.query_strategies=='random':
    ...
    raise NotImplementedError('wrong query strategies')
  ...
  # ACTIVE LEARNING
  pool = []
  imdb_guide_AL = args.imdb_guide + '_AL'
  image_set_file_g = os.path.join(imdb_g._data_path, 'ImageSets', 'Main',
                                imdb_g._image_set + '.txt')
  image_set_file_AL = os.path.join(imdb_g._data_path, 'ImageSets', 'Main',
                                   imdb_g._image_set + '_AL.txt')
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    # Add queries to guide
    pool = Query.query(dataloader_tnf, fasterRCNN, pool, epoch)
    shutil.copy(image_set_file_g, image_set_file_AL)
    f = open(image_set_file_AL, 'a+')
    for x in pool:
    ...
    cache_file_AL = os.path.join(imdb_g.cache_path, imdb_guide_AL + '_gt_roidb.pkl')
    if os.path.isfile(cache_file_AL):
        os.remove(cache_file_AL)

    # define new dataloader
    imdb_g, roidb_g, ratio_list_g, ratio_index_g = combined_roidb(imdb_guide_AL)
    train_size_g = len(roidb_g)
    ...
    for step in range(iters_per_epoch_s):
      ...
      rois_label2, RCNN_loss_img2, ssda_mme = fasterRCNN(im_data, im_info, gt_boxes, num_boxes,
                                               need_backprop=need_backprop, dc_label=dc_label)
      loss =  RCNN_loss_img2.mean() + ssda_mme.mean() # + RCNN_loss_ins2.mean()
      loss_temp += loss.item()
