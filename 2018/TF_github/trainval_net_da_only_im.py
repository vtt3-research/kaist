...
if __name__ == '__main__':
  ...
  if args.dataset == "original_n_foggy":
  ...
  if args.resume:
    load_name = os.path.join(output_dir,
                             'faster_rcnn_only_im_{}_{}_{}.pth'.format(args.checksession, args.checkepoch, args.checkpoint))
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      need_backprop = data2[4]
      dc_label.data.resize_(data2[5].size()).copy_(data2[5])
      ...
      if step % args.disp_interval == 0:
        ...
        if args.mGPUs:
          ...
          loss_rcnn_img2 = RCNN_loss_img2.mean().data[0]
        ...
        print("\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f, source_rcnn_img: %4f, target_rcnn_img: %4f" \
              % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box, loss_rcnn_img, loss_rcnn_img2))
    ...
    if epoch%args.checkpoint_interval==0:
      save_name = os.path.join(output_dir, 'faster_rcnn_only_im_{}_{}_{}.pth'.format(args.session, epoch, step))
      save_checkpoint({
      ...
      print('%02d hours %02d minutes %02d secnods' % (duration//3600, (duration%3600)//60, (duration%3600)%60))
      save_start = time.time()
